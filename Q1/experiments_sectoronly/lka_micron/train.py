...
# ============================================================
# SCRIPT (lka micron): conditioning on one-hot(ADM1) +
#                        one-hot(sector), PSU sampling by exact
#                        PSU-per-ADM1 levels:
...
OUTPUT_DIR = "/data/shared/fsibilla/clean_code/Q1/experiments_sectoronly/lka_micron/results"
...
# -----------------------
# CONDITIONS: one-hot(ADM1) + one-hot(sector)
# -----------------------
adm1_dummies_full = pd.get_dummies(df[adm1_name_col], prefix=adm1_name_col)
sector_dummies_full = pd.get_dummies(df[sector_col], prefix=sector_col)

cond_df_full = pd.concat([adm1_dummies_full, sector_dummies_full], axis=1)
...
for psu_per_adm1 in PSU_PER_ADM1_LEVELS:
    ...
    for seed in SEEDS:
        ...
        # -----------------------
        # PSU sampling:
        #   1) always 1 PSU per ADM1
        #   2) first PSU in each ADM1 chosen with sector proportional
        #   3) extra PSUs drawn sequentially from remaining (ADM1, sector)
        #      proportionally to remaining cell sizes
        # -----------------------
            ...
            json.dump(
                {
                    ...
                    "conditioning": "onehot_adm1_plus_onehot_sector",
        ...
        # kept same columns for compatibility
        ...
        # -----------------------
        # SAVE complete_lka_scaled
        # -----------------------
        X_full = df[target_cols].values.astype(float)
        X_full_s = x_scaler.transform(X_full)

        complete_lka_scaled = df.copy()
        complete_lka_scaled[target_cols] = X_full_s

        # kept same columns for compatibility
        complete_lka_scaled["train_frac_psu"] = psu_per_adm1
        complete_lka_scaled["train_percent_psu"] = psu_per_adm1

        complete_lka_scaled["seed"] = seed
        complete_lka_scaled["scaler_method"] = SCALE_METHOD

        complete_scaled_path = os.path.join(
            seed_dir,
            f"full_lka_scaled_train{psu_per_adm1}_seed{seed}.csv"
        )
        complete_lka_scaled.to_csv(complete_scaled_path, index=False)
