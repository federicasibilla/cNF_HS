...
# ============================================================
# SCRIPT (nga micron): conditioning on continuous ADM1 features +
...
OUTPUT_DIR = "/data/shared/fsibilla/clean_code/Q1/stationarity/nga_micron/results"
...
_all_adm1s = np.sort(psu_info_df["psu_modal_adm1"].unique())
...
# Filter PSU pool to in-sample ADM1s only — these are the only ADM1s the model
# can see during training. Generation still happens for the FULL dataset, so
# evaluate.ipynb can compare EMD on in-sample vs held-out ADM1s.
psu_info_df = psu_info_df.loc[psu_info_df["psu_modal_adm1"].isin(_in_sample_adm1s)].copy()

# Save the split so evaluate.ipynb can label cells correctly
if SAVE_SPLIT_FILE:
...
for psu_per_adm1 in PSU_PER_ADM1_LEVELS:
    ...
    for seed in SEEDS:
        ...
        print(f"  -> Saved complete_nga_scaled to:\n     {complete_scaled_path}")
