...
COND_BASE_COLS = ["entropy_2", "wscore_1"]
ADM1_COL       = "adm1name"
HH_ID_COL      = "id"
SCALE_METHOD   = "iqr"
...
need_cols = TARGET_COLS + COND_BASE_COLS + [ADM1_COL, HH_ID_COL]
df_full   = df_full.dropna(subset=need_cols).copy()

for c in TARGET_COLS + COND_BASE_COLS:
    df_full[c] = df_full[c].astype(float)

df_full[ADM1_COL]  = df_full[ADM1_COL].astype(str)
df_full[HH_ID_COL] = df_full[HH_ID_COL].astype(str)

# build condition matrix once — same columns will be reused for every model
cond_df_full = df_full[COND_BASE_COLS].copy()
...
for seed in SEEDS:
    ...
    seed_dir     = os.path.join(RESULTS_DIR, f"train_{HH_LEVEL}_scaled", f"seed_{seed}_scaled")
    ...
    # -------------------------------------------------------------------------
    train_path = os.path.join(seed_dir, f"train_subset_{HH_LEVEL}_seed{seed}_scaled.csv")
    df_train   = pd.read_csv(train_path)

    df_train[ADM1_COL]  = df_train[ADM1_COL].astype(str)
    df_train[HH_ID_COL] = df_train[HH_ID_COL].astype(str)

    C_train   = df_train[COND_BASE_COLS].values.astype(float)
    Y_train_s = df_train[TARGET_COLS].values.astype(float)   # already scaled

    # load scaler fitted on this training subset
    scaler_path = os.path.join(seed_dir, f"x_scaler_{SCALE_METHOD}_train{HH_LEVEL}_seed{seed}.json")
    with open(scaler_path) as f:
        scaler = MonotoneLinearScaler(method=SCALE_METHOD).from_dict(json.load(f))

    print(f"  Training rows: {df_train.shape[0]}  |  Households: {df_train[HH_ID_COL].nunique()}")
    ...
    std = std_s * scaler.scale_                        # std in raw units
