...
#
#                        ADDED: iterative feature removal outer loop
#                        - define FEATURE_REMOVAL_LEVELS in config
#                        - each level specifies which cond_cols to drop
#                        - results are saved in separate subdirs per level
# ============================================================

# -----------------------
# CONFIG
# -----------------------
DATA_PATH = "/data/shared/fsibilla/clean_code/Q1/experiments/nga_mics/full.csv"
OUTPUT_DIR = "/data/shared/fsibilla/clean_code/Q1/feature_importance/nga_mics/results"
...
# ---------------------------------------------------------------
# FEATURE REMOVAL LEVELS
# Each entry defines a named experimental condition.
# "drop_cond_cols" lists which columns from cond_cols_adm1 to remove.
# "sector" is always kept (handled separately as one-hot).
# Add or remove entries here to define your ablation schedule.
# ---------------------------------------------------------------
FEATURE_REMOVAL_LEVELS = [
    # --- size 1: remove one block ---
    {
        "name": "drop_entropy2",
        "drop_cond_cols": ["entropy_1"]
    },
    {
        "name": "drop_rwi2",
...
# sector dummies are always computed once (sector col never removed)
sector_dummies_full = pd.get_dummies(df[sector_col], prefix=sector_col)

os.makedirs(OUTPUT_DIR, exist_ok=True)

# ==============================================================
# OUTER LOOP: feature removal levels
# ==============================================================
for feature_level in FEATURE_REMOVAL_LEVELS:
    feat_name = feature_level["name"]
    drop_cols = set(feature_level["drop_cond_cols"])

    # Recompute active conditioning columns for this level
    # sector is always kept — it is handled as one-hot separately
    active_cond_cols = [c for c in cond_cols_adm1 if c not in drop_cols]
    active_base_cols = [c for c in active_cond_cols if c != sector_col]

    cond_base_full = df[active_base_cols]
    cond_df_full = pd.concat([cond_base_full, sector_dummies_full], axis=1)
    C_full = cond_df_full.values.astype(float)

    print(f"\n{'='*60}")
    print(f"Feature level : {feat_name}")
    print(f"Active cond cols : {active_cond_cols}")
    print(f"Dropped cond cols: {sorted(drop_cols) if drop_cols else 'none'}")
    print(f"{'='*60}")

    # ==============================================================
    # MIDDLE LOOP: PSU-per-ADM1 levels
    # ==============================================================
    for psu_per_adm1 in PSU_PER_ADM1_LEVELS:
        print(f"\n  ==============================")
        print(f"  PSU-per-ADM1 level: {psu_per_adm1}")
        print(f"  ==============================")

        # Output dir is namespaced by feature level to avoid overwrites
        level_dir = os.path.join(OUTPUT_DIR, feat_name, f"train_{psu_per_adm1}_scaled")
        ...
        # ==============================================================
        for seed in SEEDS:
            print(f"\n  --- Seed {seed} | PSU level {psu_per_adm1} | Features: {feat_name} ---")
            ...
            print(
                ...
                f"(target {n_psu_train_target}; level={psu_per_adm1} PSU/ADM1), "
            ...
            with open(scaler_path, "w") as f:
                json.dump(
                    {
                        "feature_level": feat_name,
                        "active_cond_cols": active_cond_cols,
                        "dropped_cond_cols": sorted(drop_cols),
            ...
            print(f"  -> Saved training data (scaled) to:\n     {subsample_scaled_path}")
            ...
            print(f"  -> Saved generated pool (raw) to:\n     {out_raw_path}")
            ...
            print(f"  -> Saved generated pool (scaled) to:\n     {out_scaled_path}")

            # -----------------------
            # SAVE FULL DATASET (SCALED TARGETS)
            # -----------------------
            ...
            complete_eth_scaled["feature_level"] = feat_name
            ...
            complete_scaled_path = os.path.join(
                seed_dir,
                f"full_nga_scaled_train{psu_per_adm1}_seed{seed}.csv"
            )
            complete_eth_scaled.to_csv(complete_scaled_path, index=False)
            print(f"  -> Saved full dataset (scaled) to:\n     {complete_scaled_path}")
