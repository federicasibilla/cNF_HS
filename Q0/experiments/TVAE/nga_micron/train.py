...
OUTPUT_DIR = "/data/shared/fsibilla/clean_code/Q0/experiments/TVAE/nga_micron/results"
...
# This is included in training, so TVAE learns the joint distribution
# of targets + adm1. We do NOT condition on it during generation.
adm1_col = "adm1name"
...
def main():
    ...
    # Keep only columns used by TVAE:
    ...
    for seed in SEEDS:
        ...
        # Train TVAE on full cleaned dataset
        ...
        print("-> Fitting TVAE on FULL dataset (targets + adm1)...")
        synthesizer.fit(train_df)

        # Save synthesizer if possible
        try:
            synthesizer.save(os.path.join(seed_dir, "tvae_synthesizer.pkl"))
            print("-> Saved TVAE synthesizer.")
        except Exception as e:
        ...
        # Save cleaned real data used for training
        real_out = train_df.copy()
        real_out["seed"] = seed
        real_out_path = os.path.join(seed_dir, f"full_nga_cleaned_used_for_training_seed{seed}.csv")
